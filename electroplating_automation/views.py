    ...
    def perform_shift_operations_new_tank(self,current_rack, myRacks, myTanks, myCranes):
        ...
        if ops_resultCheck_map['result_id'] == 1:
            ...
            if result_n['result_id'] == 2:
                crane_op = CraneOperation(0, myFirstCrane.tank_number, 1, new_rack_index)
...
# if that is the case, then i might not need to deupdate
    def check_shift_operations(self,sortedRacks, myTanks, myCranes):
        ...
        if current_tank.tank_number == 7:
            self.final_racks_list = sortedRacks
            return {'result_id': 2}
            time_left_for_other_rack_after_shifting_current_rack = mins_available_other_rack - ((mins_available_current_rack - time_taken_to_get_crane_to_current_tank) + self.final_rack_shift_time + self.calculate_tank_shift_time(current_tank.tank_number, other_tank.tank_number) + 2*self.tank_cross_time)
    ...
    def calculate_racks_and_cranes(self, myRacks, myCranes):
        ...
        if no_of_racks == 0:
            ...
            new_rack = RackTrack(new_rack_index, 1, (float(first_tank.immersion_time_mins) - self.rack_pick_time_mins), -1, -1, -1)
        ...
        # first lets try this process by adding a new rack
            ...
            if myRacks[no_of_racks - 1].tank_number !=1 :
                ...
                sub_new_rack = RackTrack(new_rack_index, 1, (float(first_tank.immersion_time_mins) - self.rack_pick_time_mins), entry_moment_shift_time_available, current_rack.tank_number, current_rack.remaining_tank_time)
            ...
            if new_result['result_id'] != 2:
                if is_new_rack_added:
                    ...
                    #self.check_data.append(new_rack_index)
                    #self.check_data.append(self.serialize_racks(myRacks))
                    if myRacks[new_rack_index].entry_moment_shift_time_available < entry_moment_shift_time_available_store and new_rack_index == shifted_rack_index:
                        taken_shift_time = entry_moment_shift_time_available_store - myRacks[new_rack_index].entry_moment_shift_time_available
                        for rack in myRacks:
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({'message': 'success', 'rack_data': self.check_data, 'crane_data': craneJSON})
